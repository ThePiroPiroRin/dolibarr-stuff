...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
	...
	if ($action == 'create' && $usercancreate) {
		...
		//!!! modified code to autofill the ref with the next value !!! Chaimae Lekhzoumy
		//query to find the ref value right before the last 5 rows
		$sql = "select ref from llx_product order by ref desc limit 1 offset 5;"; // !!! if the last 5 rows are deleted/added/modified just change the offset HERE !!!
		$res = $db->query($sql);
		$lastIdNumber = $db->fetch_array($res);
		if ($lastIdNumber) {
			$refValue = ++$lastIdNumber['ref']; //increment for next access
		} else {
			$refValue = 'P0001'; //if no value is found set to P0001
		}
		if (empty($conf->global->PRODUCT_GENERATE_REF_AFTER_FORM)) {
			print '<tr>';
			$tmpcode = '';
			...
			$inputValue = GETPOST('ref', 'alphanohtml') ? GETPOST('ref', 'alphanohtml') : $refValue;
    		// If the user manually changes the input it will use the changed value. Otherwise, it will use $refValue but the next incrementation will still be of whichever is 6th from the bottom
			print '<td class="titlefieldcreate fieldrequired">'.$langs->trans("Ref").'</td><td><input id="ref" name="ref" class="maxwidth200" maxlength="128" value="'.dol_escape_htmltag($inputValue).'">';
			if ($refalreadyexists) {
