...
//!!! changed sql query to select the corresponding fournisseur ref if it exists and display it instead of the tiers full name next to the tiers alias chaimae lekhzoumy!!!
$sql .= ' f.rowid as id, f.ref, concat(IFNULL(ff.ref, "")," ", s.name_alias) as name, f.ref_client, f.type, f.note_private, f.note_public, f.increment, f.fk_mode_reglement, f.fk_cond_reglement, f.total as total_ht, f.tva as total_vat, f.total_ttc,';
...
$sql .= ' s.rowid as socid, s.email, s.town, s.zip, s.fk_pays, s.client, s.fournisseur, s.code_client, s.code_fournisseur, s.code_compta as code_compta_client, s.code_compta_fournisseur,';
...
//added left join to access fourn table
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."facture_fourn as ff on (f.ref = ff.ref_supplier)";
